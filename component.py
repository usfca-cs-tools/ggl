...
class ComponentInputProxy:
    ...
    def __init__(self, wrapped_input, instance_id):
        ...
        self.outputs = NodeOutputs(['0'], self)  # Proxy outputs go to wrapped input
    ...
    def propagate(self, value=0):
        ...
        if input_edge:
            ...
            self.wrapped_input.value = input_edge.value
            logger.debug(f"InputProxy {self.label} setting wrapped input {self.wrapped_input.label} value to {input_edge.value}")
            # Return the wrapped input to be processed in the next iteration
            return [self.wrapped_input]
        logger.warn(f"InputProxy {self.label} has no input edge!")
        return []
...
class ComponentOutputProxy:
    ...
    def __init__(self, wrapped_output, instance_id):
        ...
        self.outputs = NodeOutputs(['0'], self)  # Provide to external connections
...
class ComponentInstance:
        ...
        for template_node in self.template.circuit.all_nodes:
            ...
            for output_name, template_edges in template_node.outputs.points.items():
                for template_edge in template_edges:
                    ...
                    if dest_points:
                        dest_point = dest_points[0]  # Get first (and should be only) destination
    ...
    def get_all_nodes(self):
        ...
        """
        all_nodes = []
        all_nodes.extend(self._input_nodes.values())      # Internal Input nodes
        all_nodes.extend(self._output_nodes.values())     # Internal Output nodes
        all_nodes.extend(self._internal_nodes)            # Internal logic nodes
        all_nodes.extend(self._input_proxies.values())    # External input proxy nodes
        all_nodes.extend(self._output_proxies.values())   # External output proxy nodes
        return all_nodes
